 ...
 * is treated as exclusive, matching Stay's checkout semantics elsewhere
 * (computeAccommodationGaps.ts) — a reservation ending exactly on the trip's
 * end_date is a normal same-day changeover, not "outside". When both start
 * and end are out of range, start takes priority — the caller uses this to
