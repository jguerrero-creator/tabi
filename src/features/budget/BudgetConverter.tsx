...
const EMPTY_STATE: ConverterState = { amount: '', sourceCurrency: '', rate: '' }
...
function loadState(tripId: string): ConverterState {
  try {
    ...
    return {
      ...
      sourceCurrency: typeof parsed.sourceCurrency === 'string' ? parsed.sourceCurrency : '',
...
export function BudgetConverter({ tripId, tripCurrency }: BudgetConverterProps) {
  ...
  const result = canCompute ? parsedAmount * parsedRate : null
      ...
      {open && (
        <div className="space-y-2 border-t border-slate-200 px-4 py-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            <Field label={strings.budgetConverter.currencyPlaceholder} className="text-xs">
              <input
                type="text"
                value={state.sourceCurrency}
                onChange={(event) =>
                  setState((prev) => ({ ...prev, sourceCurrency: event.target.value.toUpperCase() }))
                }
                placeholder={strings.budgetConverter.currencyPlaceholder}
                maxLength={8}
          ...
          <Field
            label={strings.budgetConverter.rateHint(state.sourceCurrency.trim() || '1')}
            className="text-xs"
          >
            <input
              type="number"
